    ...
    for (String key : object.keySet()) {
      ...
      if (value.isJsonObject()) {
        ...
        if (values.size() == 1 && values.get(0).startsWith("$")) {
          map.addValue(key, deserialize(valueObject, values, context));
          continue;
        }
        ...
        for (JsonElement element : array) {
          if (element.isJsonObject()) {
            ...
            if (values.size() == 1 && values.get(0).startsWith("$")) {
              objects.add(deserialize(elemObject, values, context));
              continue;
            }
          }
          objects.add(context.deserialize(element, Object.class));
        }
        map.addValue(key, objects);
        continue;
      }

...
public class ChannelsCommands {
     ...
     */
    @Command(aliases = "add", description = "add.description", permission = @Perm(node = "starfish.add"))
    public Result add(BotUser user, Message message, CommandContext context) {
        List<IMentionable> mentions = message.getMentions(Message.MentionType.ROLE, Message.MentionType.USER);
        ...
        if (ticket == null) {
            ...
            return new Result(user.getLocaleFile().get("add.members-added-to-ticket", ticket.getPlaceholders()));
        }
    }
     ...
     * @return the result of the command execution
     */
    @Command(aliases = "remove", description = "remove.description", permission = @Perm(node = "starfish.remove"))
    public Result remove(BotUser user, Message message, CommandContext context) {
        ...
        if (ticket == null) {
            ...
            return new Result(user.getLocaleFile().get("remove.members-remove-to-ticket", ticket.getPlaceholders()));
        }
    }

...
class TicketPanelReactionResponse implements ReactionResponse {
    ...
    public void onReaction(@NotNull MessageReactionAddEvent event) {
        ...
        AnnotatedCommand command = Validate.notNull(manager.getCommand("new"), "Ticket Panel: 'new' command was not registered!");
        Result result = command.execute(new CommandContext(event.getChannel().retrieveMessageById(event.getMessageIdLong()).complete(), event.getUser(), new String[]{"-new"}, event.getTextChannel(), manager.getMessagesProvider(), manager.getRegistry(), "new"));
        EmbedFactory.fromResult(result, listener, null).send(event.getTextChannel(), listener.getConsumer(result));
    }

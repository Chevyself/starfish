...
public class TicketTranscript implements StarfishCatchable {
    ...
    public TicketTranscript(@NonNull Ticket ticket) throws IOException {
        String url = CoreFiles.validatePath(CoreFiles.currentDirectory() + "/transcripts/ticket-" + ticket.getId() + ".txt");
        CustomFormatter formatter = new CustomFormatter(Starfish.getLanguageHandler().getDefault().get("transcript-log"));
        FileHandler fileHandler = new FileHandler(url, true);
        fileHandler.setFormatter(formatter);
        this.file = CoreFiles.getOrCreate(url);
        this.id = ticket.getId();
        this.logger = LoggerFactory.start("Ticket-" + ticket.getId(), LoggerFactory.getConsoleHandler(formatter), fileHandler);
    }

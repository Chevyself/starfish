...
public class TicketTranscriptHandler implements StarfishHandler {
    ...
    public void onMessageReceivedEvent(GuildMessageReceivedEvent event) {
        ...
        if (ticket != null) {
            ...
            Map<String, String> placeholders = getPlaceholders(event, ticket);
    ...
    private TicketTranscript getTranscript(@NonNull Ticket ticket) {
        return Starfish.getCache().getOrSupply(TicketTranscript.class, transcript -> transcript.getId() == ticket.getId(), () -> {
            try {
                return new TicketTranscript(ticket).cache();
            } catch (IOException e) {
                Starfish
                        .getFallback().process(e, "Transcript for Ticket-" + ticket.getId() + " could not be initialized");
                return null;
            }
        });
    }
